...
int find_index_sorted(double x, std::vector<double> &data, int start = 0,
                      int end = -1) { // Basic dichotomy implementation
  ...
  if (data[middle] <= x) {
    return find_index_sorted(x, data, middle, end);
  } else {
    return find_index_sorted(x, data, start, middle);
  }
}
std::vector<int> closest_q_index_in_sorted(
  ...
  // data
  if (q > data.size()) {
    qDebug() << "closest_q_index_in_sorted unsuccessful call : asking to "
                "retrieve more values than the length of the vector (q: "
             << QString::number(q)
             << " > data size: " << QString::number(data.size()) << ")\n";
    throw std::invalid_argument("Asking to retrieve more values than the "
                                "length of the vector (q > data.size())");
  }
...
std::vector<double> weighted_least_squares(std::vector<double> dataX,
  ...
  std::vector<double> coefficients =
      std::vector<double>{detA / detMat, detB / detMat, detC / detMat};
  return coefficients;
}
...
double g_hat_x(std::vector<double> dataX, std::vector<double> dataY, double x) {
  ...
  double f = 0.1;
  int q = std::max(
      std::min(3.0, static_cast<double>(dataX.size())),
      f * (static_cast<double>(
              dataX.size()))); // Number of neighbors CONSTANT choose Q thanks
                               // to M diagram (=q/n) in loess details)
  std::vector<int> closest_xi_index = closest_q_index_in_sorted(x, q, dataX);
  double furthest_distance = std::max(
      std::abs(x - dataX[closest_xi_index[0]]),
  ...
  for (int i = 0; i < closest_xi_index.size(); i++) {
    weights.push_back(
        neighborhood_weight(dataX[closest_xi_index[i]], x, furthest_distance));
...
auto DataAnalysis::stl_regression(
    std::vector<double> dataX,
    std::vector<double>
        dataY) { // Definitely need to change this and the hpp file
  // TODO : Do the testing
  // TODO : replace find_index by a composition of stl's lower_bound and
  // distance to make the code better
