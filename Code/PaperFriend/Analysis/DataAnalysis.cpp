...
double DataAnalysis::cov(const std::vector<double> &X,
  ...
  for (unsigned int i = 0; i < std::min(X.size(), Y.size());
       ++i) { // Just in case the two have different sizes but shouldn't be the
              // case.
...
// Robert B. Cleveland, William S. Cleveland, Jean E. McRae, and Irma Terpenning : STL : A Seasonal-Trend Decomposition Procedure Based on Loess.

bool smallerEqual(double a, double b){return a<=b;}

int find_index_sorted(double x, std::vector<double>& data){ // Basic dichotomy implementation
    // Returns the biggest index corresponding in data to a number <= x   (for ex, find_index(2.5, {0, 1, 2, 3}) returns 2)
    if (data.size()==0) {qDebug() << "Data of length 0 on find_index_sorted";}
    if (x < data[0]) {return 0;}
    if (data[data.size()-1] < x) {return data.size()-1;}
    return std::distance(data.begin(),  std::lower_bound(data.begin(), data.end(), x, smallerEqual))-1;
}
...
std::vector<double> weighted_least_squares(std::vector<double> dataX, std::vector<double> dataY, std::vector<double> weights = {-1}){
    ...
    double sum_x=0, sum_x2=0, sum_x3=0, sum_x4=0, sum_x2y=0, sum_xy=0, sum_y=0, sum_1=0, xwi, xi, yi;
...
double loess_x (std::vector<double> dataX, std::vector<double> dataY, int q, double x, std::vector<double> robust_w = {-1}){
    ...
    for (int i=0; i<closest_xi_index.size(); i++){
        ...
        if (robust_w[0] == -1){
            ...
            weights.push_back(neighborhood_weight(dataX[closest_xi_index[i]], x, furthest_distance) * robust_w[closest_xi_index[i]]);
        }
    ...
    double value = eval_poly(weighted_least_squares(closest_xi, closest_yi, weights), x);
    return value;
}
...
std::vector<std::vector<double>> DataAnalysis::stl_regression(std::vector<double> dataX, std::vector<double> dataY, int seasonal_length){ // Definitely need to change this and the hpp file
    ...
    std::vector<std::vector<double>> all_cycle_subseries_detrended(np); // list of subseries in detrended
    std::vector<std::vector<double>> X_all_cycle_subseries_detrended(np);
    std::vector<std::vector<double>> all_cycle_subseries_smoothed(np); // list of subseries smoothed
    std::vector<double> Cycle_subseries_smoothed(N + 2*seasonal_length, 0); // regroup all subseries
    ...
    for (int outer_loop = 0; outer_loop<no; outer_loop++) {
        ...
        for (int inner_loop = 0; inner_loop<ni; inner_loop++){
            ...
            // Step 2 : Cycle-subseries smoothing (cycle-subseries = seasonal_length observations)
            for (int i=0; i<np; i++){
            ...
            for (int i=0; i<dataX.size(); i++){
                all_cycle_subseries_detrended[static_cast<int>(dataX[i])%np].push_back(Detrended[i]);
                X_all_cycle_subseries_detrended[static_cast<int>(dataX[i])%np].push_back(static_cast<int>(dataX[i])/np);
            }
            ...
            for (int subseries=0; subseries<np; subseries++){
                for (int i= -1 ; i<nb_seasons + 1; i++){ //start at -1, end at nb_seasons + 1
                    all_cycle_subseries_smoothed[subseries].push_back(loess_x(X_all_cycle_subseries_detrended[subseries], all_cycle_subseries_detrended[subseries], ns, i, robustness_weight));
                }
            }
            ...
            for (int i=0; i<N+ 2*np; i++){
                Cycle_subseries_smoothed.push_back(all_cycle_subseries_smoothed[i%np][i/np]);
            }
        ...
        for (int i=0; i<dataX.size(); i++){
            ...
            Abs_Remainder[i] = std::abs(dataY[i] - Trend[dataX[i]] - Seasonality[dataX[i]]);
        }

        if (dataX.size()%2 == 0){
            std::nth_element(Abs_Remainder.begin(), Abs_Remainder.begin()+(dataX.size() / 2), Abs_Remainder.end());
            std::nth_element(Abs_Remainder.begin(), Abs_Remainder.begin()+((dataX.size() / 2)-1), Abs_Remainder.end());
            h = 3*( Abs_Remainder[dataX.size()/2] + Abs_Remainder[(dataX.size()/2)-1] );
        }
        else{
            std::nth_element(Abs_Remainder.begin(), Abs_Remainder.begin()+((dataX.size()-1) / 2), Abs_Remainder.end());
