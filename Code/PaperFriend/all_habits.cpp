    ...
    ui(new Ui::All_Habits)
{
    ...
    if (current_habits.size()>0){
        for (unsigned long i = 0; i < current_habits.size(); i++) {
            text_to_insert += current_habits[i][0] + ", " + current_habits[i][1] + ", done " + current_habits[i][2] + " time(s) in a row.\n";
        }
...
void All_Habits::on_save_habit_button_clicked() {
    ...
    if (duplicates){
        QMessageBox::warning(this, "", "One or more habits entered already exist.", QMessageBox::Close);
    }
    else if (duplicates_amoung_entered_habits) {
        QMessageBox::warning(this, "", "One or more habits entered are the same.", QMessageBox::Close);
    }
    else {
