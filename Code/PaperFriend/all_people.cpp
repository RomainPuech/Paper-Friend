...
all_people::all_people(std::vector<Friend> &vector_friend, QWidget *parent)
    : vector_friends(vector_friend), QDialog(parent), ui(new Ui::all_people) {
  ui->setupUi(this);
  int PersonCellNumberTotal = 0;
  this->add_previous_cells();
}

all_people::~all_people() { delete ui; }

void all_people::on_add_people_button_clicked() { addNewCell(); }

void all_people::addNewCell(QString cellText, QString cellName, int type) {
  people_cell *new_person_cell =
      new people_cell(this); // Create a new people_cell.
  ui->people_cell_layout->addWidget(
      new_person_cell); // add a widget in the people_cell_layout.
  connect(new_person_cell, SIGNAL(closeThisCell(int)), this,
          SLOT(closeCell(int)));
  new_person_cell->setAttribute(
      Qt::WA_DeleteOnClose, true); // here we set an attribute so whenever we
                                   // close the cell it is indeed deleted.
  new_person_cell->PersonCellNumber =
      PersonCellNumberTotal; // here in the new_person_cell I defined the person
                             // cell number to be equal to the total person cell
                             // number.
  new_person_cell->set_person_cell_name(cellName);
  PersonallCellPtr.append(new_person_cell); // here I append this new persson
...
void all_people::add_previous_cells() {
  for (long long unsigned i = 0; i < vector_friends.size(); i++) {
    addNewCell("", QString::fromStdString(vector_friends.at(i).get_name()),
               vector_friends.at(i).get_type());
...
void all_people::on_save_people_button_clicked() {
  ...
  reply = QMessageBox::information(
      this, "Save Confirmation", "All your people are saved.", QMessageBox::Ok);
...
void all_people::closeEvent(QCloseEvent *event) {
  if (PersonallCellPtr.size() != vector_friends.size()) {
    QMessageBox::StandardButton answr_btn = QMessageBox::warning(
        this, tr("Paper friend"), tr("Have you saved your people ?"),
    ...
    for (int i = 0; i < PersonallCellPtr.size(); ++i) {
      ...
      if (name_person_acp.toStdString() != name_person_vec ||
          type_person_acp != type_person_vec) {
        QMessageBox::StandardButton answr_btn = QMessageBox::warning(
            this, tr("Paper friend"), tr("Have you saved your people ?"),
      ...
      if (type_person_acp == 0) {
        QMessageBox::StandardButton answr_btn = QMessageBox::warning(
            this, tr("Paper friend"), tr("Please enter a person category."),
            QMessageBox::Yes | QMessageBox::Yes | QMessageBox::No,
            QMessageBox::No);
      ...
      if (name_person_acp.toStdString() == "Person name") {
        QMessageBox::StandardButton answr_btn = QMessageBox::warning(
            this, tr("Paper friend"), tr("Please enter the person's name."),
            QMessageBox::Yes | QMessageBox::Yes | QMessageBox::No,
            QMessageBox::No);
