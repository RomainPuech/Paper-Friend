...
{   
    ...
    for(auto e: entries){
        ...
        else if(level!=current_mood_level){//the mood level changed, we need to change the line's color, so create a new series (a series can only have one single color)
        ...
        }//if the mood level remained the same, we just add the point to the current series
...
void DynamicGraph::display(QLayout *layout) const
{
    ...
    mood_chart->addSeries(visible_green_points);
    mood_chart->addSeries(visible_orange_points);
    mood_chart->addSeries(visible_red_points);
